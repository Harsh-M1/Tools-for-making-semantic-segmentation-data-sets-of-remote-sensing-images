...
def Label2grey(Origin_SegmentationClass_path,Out_SegmentationClass_path,Origin_Point_Value,Out_Point_Value):
    ...
    for png_name in tqdm(png_names):
        ...
        if not os.path.exists(png_file_name):
            raise ValueError(
                "未检测到标签图片%s，请查看具体路径下文件是否存在以及后缀是否为png。" % (png_file_name))
...
if __name__ == "__main__":
	...
	# -----------------------------------------------------------------------------------#
	Origin_SegmentationClass_path = r"G:\mutilmode_data\ori_image_label\changhai\muldataset\labels_ori"
	Out_SegmentationClass_path = r"G:\mutilmode_data\ori_image_label\changhai\muldataset\Labels"
	...
	# -----------------------------------------------------------------------------------#
	Label2grey(Origin_SegmentationClass_path,Out_SegmentationClass_path,Origin_Point_Value,Out_Point_Value)